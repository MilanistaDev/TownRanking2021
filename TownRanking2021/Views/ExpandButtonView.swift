    ...
    static var previews: some View {
        Group {
            ExpandButtonView(selection: .rent, isExpanded: .constant(false))
                ...
                .previewDisplayName("!isExpanded, Rent, Light mode")
            ExpandButtonView(selection: .buy, isExpanded: .constant(false))
                .previewLayout(PreviewLayout.sizeThatFits)
                .padding()
                .background(Color(.systemBackground))
                .environment(\.colorScheme, .dark)
                .previewDisplayName("!isExpanded, Buy, Dark mode")
            ExpandButtonView(selection: .rent, isExpanded: .constant(true))
                ...
                .environment(\.colorScheme, .light)
                .previewDisplayName("isExpanded, Rent, Light mode")
            ExpandButtonView(selection: .buy, isExpanded: .constant(true))
                .previewLayout(PreviewLayout.sizeThatFits)
                .padding()
                .background(Color(.systemBackground))
                .environment(\.colorScheme, .dark)
